...
type MonitorService interface {
	List(ctx context.Context) (res *api.MonitorSearchRes, err error)
}
...
func NewSysMonitorService() MonitorService {
	return &monitorService{
		startTime: time.Now(),
	}
}

func (svc *monitorService) List(ctx context.Context) (res *api.MonitorSearchRes, err error) {
	...
	goUsed = gomem.Sys
	float, _ := strconv.ParseFloat(fmt.Sprintf("%.2f", float64(goUsed)/float64(memTotal)*100), 64)
	if err != nil {
		return nil, err
	}
	goUsage = float
	...
	//gtime.Date()
	goStartTime := svc.startTime //启动时间

	goRunTime := time.Now().Sub(svc.startTime).Seconds() //运行时长（秒）
	...
	res = &api.MonitorSearchRes{
		"cpuNum":          cpuNum,
		"cpuUsed":         cpuUsed,
		"cpuAvg5":         fmt.Sprintf("%f", cpuAvg5),
		"cpuAvg15":        fmt.Sprintf("%f", cpuAvg15),
		"memTotal":        memTotal,
		"goTotal":         goTotal,
		"memUsed":         memUsed,
		"goUsed":          goUsed,
...
func (svc *monitorService) getLocalIP() (ip string, err error) {
	var addrs []net.Addr
